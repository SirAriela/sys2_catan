...
namespace game
{
    ...
    {
        if (point1->getBuilding() == BuildingType::None && point2->getBuilding() == BuildingType::None)
        {
            if (point1->getroadsPossible().size() == 0 || point2->getroadsPossible().size() == 0)
                throw "No building on the points";
        }
        if (hasResources(ROAD_COST))
        {
            ...
            Road road(point1, point2);
            roads.push_back(road);
            // add the road to the points
            point1->getroadsPossible().push_back(*point2);
            point2->getroadsPossible().push_back(*point1);
            roads.push_back(road);
        }
        else
        {
